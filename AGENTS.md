...
- **Run `mattpocock-skills:code-review` on every PR before merging, then again after
  ...
  That pass checks conformance, standards and spec, not correctness: code that
  follows every convention and implements the issue can still be wrong, and #498
  was. Two repo-local skills under `.claude/skills/` answer the other questions,
  neither required. `correctness-review` asks what breaks the diff, for any change
  that touches behaviour; `app-security-review` covers the five security classes
  this app has shipped fixes for and a generic security review excludes by policy,
  for a diff under `src/server`, `src/lib`, `infra` or any prompt or logger. Run
  your harness's generic security review beside it, or cover its categories by hand
