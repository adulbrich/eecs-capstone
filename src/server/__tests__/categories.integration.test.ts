...
describe("the cached public list (#558)", () => {
  ...
  it("shows a staff create, rename and delete on the task that made it", async () => {
    const admin = await makeUser(`cache-${Date.now()}@x.com`, "admin");
    const names = async () =>
      (await listCategoriesImpl({ domain: "project" })).rows.map((r) => r.name);
    ...
    expect(await names()).toEqual(["Rust"]);
    ...
    await deleteCategoryAs(admin, id);
    expect(await names()).toEqual([]);
  });

  it("serves a write made behind its back from the cache, which is the trade", async () => {
    // What ADR-0048 accepts: a row written by another task, or here straight
    // to the database, is not seen until the entry expires. This pins that
    // the cache is really on in this suite, so the test above means something.
    expect((await listCategoriesImpl({ domain: "project" })).rows).toEqual([]);
    await db
      .insert(categories)
