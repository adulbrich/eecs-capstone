...
/**
 * shadcn's Input OTP (new-york), written by hand rather than through the CLI
 * (#600). `InputOTPSeparator` is left out because nothing uses it.
 *
 * `OTPInput` renders one real `<input>` over the slots, so `id`, `name`,
 * `autoComplete`, `aria-*` and a `<Label htmlFor>` all land on that input, and
 * `FormData` and Playwright's `getByLabel(...).fill()` see an ordinary field.
 * The slots are drawing only.
 */
function InputOTP({
  className,
  containerClassName,
