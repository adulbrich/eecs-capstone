 ...
 * so this is the suite's inbox: a sign-in code is read out of it the way a
 * person would read it out of theirs. The `verification` row holds the same
 * code, but encrypted, and reading the database would skip the mail the test is
 * meant to prove was sent. Playwright pipes the server's output into its own
