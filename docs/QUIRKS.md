...
Better Auth enables its rate limiter only under `NODE_ENV=production` (`better-auth/dist/context/create-context.mjs`), so nothing local exercises it and no test would have caught this. Behind CloudFront and the internal ALB each hop appends an address to `X-Forwarded-For`, so the header always carries at least two entries, and with no `advanced.ipAddress.trustedProxies` Better Auth trusts only a single-entry header: it resolved no address, keyed every request on the constant `no-trusted-ip`, and shared one bucket per path across the internet, 3 sign-ins per 10 seconds for everyone, logging one warning per task start and nothing else (#519). The same resolver feeds `session.ipAddress`, which was empty for every production session. `buildAuthConfig` now reads `TRUSTED_PROXY_CIDR` into `trustedProxies`, `auth.ts` passes it through, and production refuses to boot without it (`startup-config.ts`); the chain is then walked from the right and the first entry outside the range is the viewer, so a prepended spoof is never reached. The value has to stay non-empty whatever it holds, because an empty trusted list sends Better Auth back down `if (forwardedIps.length !== 1) return null` and collapses every viewer who sits behind their own proxy into the shared bucket again.

What it does **not** do is name a hop inside the VPC. The ALB appends the CloudFront edge server's public address rather than the CloudFront VPC origin ENI, which AWS documents under [Client IP addresses](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/RequestAndResponseBehaviorCustomOrigin.html#RequestCustomIPAddresses), so `var.vpc_cidr` matches nothing in the chain and the limiter keys on a CloudFront edge server. That is wrong in both directions at once: CloudFront spreads one viewer across many edge addresses, so a limit is diluted by a factor nobody controls, and a viewer whose requests do land on one edge address gets a 429 caused by strangers. #535 corrects it at the load balancer. The counter is also per task, in memory, so whatever the number says it multiplies by the running task count and resets on every deploy. See the `trustHost` quirk above for the same chain from the origin-check side.

### The rate limit numbers are configured, not inherited

`src/lib/_internal/auth-rate-limits.ts` holds them, and `src/lib/auth.ts` passes the object straight through. Better Auth's own defaults are 100 requests per 10 seconds globally and 3 per 10 seconds for anything whose path starts with `/sign-in`, and both are wrong here for the same reason: OSU wireless NATs students into a pool of shared public addresses, so one bucket covers however many devices sit behind one pool address. The 3-per-10-seconds rule is the acute one, because `/sign-in` is a prefix and it therefore covers `/sign-in/oauth2` (ONID) and `/sign-in/social` (GitHub) as well as `/sign-in/email`. Those two mint an OAuth state and redirect; the password is typed at Microsoft or GitHub. So the default made the fourth student to click Sign in with ONID inside ten seconds a lockout, and protected nothing in exchange. All three sign-in paths are raised to 60 per 10 seconds, and `/get-session` is switched off entirely because `authClient.useSession()` calls it on every page render (136 of the 139 `/api/auth/*` requests measured in production) and it reads a session the caller already holds a cookie for.

Two traps when editing that file. Better Auth takes the **first** key that matches (`Object.keys(...).find` in `better-auth/dist/api/rate-limiter/index.mjs`), so a wildcard beside a specific path makes behaviour depend on declaration order. And its glob treats `*` as "not a slash", so `/sign-in*` matches `/sign-in` alone and never `/sign-in/email`, which is the shape most attempts at this assume works. Spell every path out. `customRules` also cannot change the key: `createRateLimitKey(ip, path)` is fixed, so no rule can count per account, and brute force protection against a shared campus address is not something this file can provide at any number.

`/sign-in/oauth2` keeps a limit rather than being switched off because Better Auth refetches the ONID discovery document from Microsoft on every call, uncached (`better-auth/dist/plugins/generic-oauth/routes.mjs`, once on sign-in and again on the callback). An unlimited path there is an amplifier pointed at the university's identity provider.

`src/lib/__tests__/auth-rate-limits.test.ts` stands up a second `betterAuth` over the memory adapter with the limiter forced on and spends each budget, which is the only thing in the repo that exercises the limiter at all. Without it CI stays green on whatever the rules happen to say, which is how #520 shipped a wrong answer.
