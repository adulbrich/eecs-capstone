...
Nitro's `node-server` entry (`node_modules/nitro/dist/presets/node/runtime/node-server.mjs`) calls srvx's `serve()` with a fixed options object. srvx spreads its `node` key into `http.createServer(options)`, which is where `keepAliveTimeout`, `headersTimeout` and `requestTimeout` live, but Nitro never passes that key and has no configuration for any of them; srvx's plugin list, which does receive the server before it is created, is Nitro's own virtual tracing list; and the four Nitro runtime hooks (`close`, `error`, `request`, `response`) never see the server. So the server keeps Node's defaults, and on Node 24 that is `keepAliveTimeout` 5000, which is below the ALB's 60 s `idle_timeout` and was the 502 of #545. Only the keep-alive is set: `headersTimeout` bounds a request whose headers have begun arriving, not an idle connection, which was measured rather than assumed (a server at 8000 against a `headersTimeout` of 3000 sent its FIN at 9006 ms). `src/nitro/keep-alive-timeout.ts` wraps `http.createServer` from the same pre-listener plugin slot `config-check.ts` uses; the values and the reasoning are in `src/lib/_internal/keep-alive-timeout.ts`, and the decision, including why a project copy of Nitro's entry through the `entry` config option was rejected, is [ADR-0039](./adr/0039-the-task-outlasts-the-load-balancer-idle-timeout.md). Reproduce with `scripts/loadtest/pooled-connection-reuse.mjs` against a `npm run build` output; note that an `http.Agent` cannot reproduce it, because Node's agent evicts a socket the moment the server closes it and a load balancer does not.
