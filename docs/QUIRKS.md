...
### Addresses are lowercase in the four columns we write, and folded everywhere else

`inventory_items.current_holder_email`, `inventory_item_status_history.holder_email`, `projects.proposer_email` and `projects.mentor_email` are stored lowercase and trimmed. `normalizeEmailAddress` in `src/lib/email-address.ts` is the only normalizer; it is reached through `holdToColumns` for the two holder columns and through `createProjectAs`, `updateProjectAs` and `updateProjectMentorshipAs` for the two project ones. `drizzle/0023_normalize_address_columns.sql` backfilled the rows written before that. #249 chose this over folding at compare time once a third comparison site appeared; ADR-0007 is the surrounding decision about proposer linking.

**`user.email` is not one of them, and its folds stay.** Better Auth normalizes it as well: `sign-up.mjs`, the `genericOAuth` callback and the admin plugin's create-user all lowercase before writing, as of 1.6.25, which a probe against the dev database confirms. That makes the folds in `resolveHold`, `lookupUserByEmailAs`, `resolveProposerId` and `mentorNameSql` defensive rather than load-bearing, and they stay defensive: it is a library's internals, not a contract, and an upgrade changing it would silently stop linking accounts. Do not read the Sentinel plugin's `emailNormalization` option as evidence to the contrary; that is about alias stripping and deduplication, and lowercasing happens without it. Only a `lower()` applied to one of the four columns above is a candidate for removal, and only after this migration has run against a deployed database, which is why this release drops none.

**Two costs, both accepted rather than overlooked.** The stored address no longer preserves the case a person typed, which staff see in the Holder column and in history rows. And `updateProjectMentorshipAs` writes the project edit log through `diffRowFields`, so the log records the normalized address, and an edit changing only case finds no changed field and returns `updated: false`. Neither is a bug to fix; read them as what shape 1 costs.
...
The decisions: `commitTransition` is the only status-history writer ([ADR-0004](./adr/0004-one-writer-per-status-history.md)), proposers link by email and only a verified address claims ([ADR-0007](./adr/0007-proposer-linking-by-email.md)), reads are staff-only when they reach an account column ([ADR-0003](./adr/0003-every-server-function-declares-its-access-level.md)), one image policy ([ADR-0009](./adr/0009-one-image-upload-policy.md)), one URL per project ([ADR-0010](./adr/0010-single-canonical-url-per-resource.md)).
