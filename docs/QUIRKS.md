...
`EMBEDDING_SOURCE_LIMIT` (`src/lib/embedding-source.ts`) truncates the source at 20,000 characters, but Titan Text Embeddings V2 refuses an input over 8,192 tokens, and no character count can guarantee a token count. The two never met: the limit was 45,000 until 2026-09-20, sized to the model's 50,000-character figure, which does not bind because the token ceiling is reached first. Nothing noticed, because every project in the corpus was under 9,000 characters. The legacy import broke that, and three DigiClips rows at 34,000 to 36,000 characters came back `Too many input tokens` at 9,999, 9,202 and 13,103. The failure is quiet by design: `refreshProjectEmbedding` catches the throw and returns `"failed"`, so the row keeps a null vector, sorts first under `recommended` (#427), and every later sweep retries it at one wasted call. 20,000 is the project form's own cap total, about 20,300 characters, made explicit, and at the worst density measured in the corpus, 2.63 characters per token on a row that is mostly links, it lands near 7,600 tokens. Denser text can still overflow; `src/lib/__tests__/embedding-source.test.ts` pins that arithmetic so raising the limit fails the suite instead of the sweep.
