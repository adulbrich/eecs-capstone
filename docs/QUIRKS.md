...
Better Auth enables its rate limiter only under `NODE_ENV=production` (`better-auth/dist/context/create-context.mjs`), so nothing local ever exercises it and no test would have caught this. In production the request path is CloudFront to an internal ALB to the task, and each hop appends an address to `X-Forwarded-For`, so the header always carries at least two entries. With no `advanced.ipAddress.trustedProxies` set, Better Auth trusts only a single-entry header, resolves no address, and keys every request on the constant `no-trusted-ip`: one bucket per path for the whole internet, and the sign-in rule is 3 per 10 seconds (#519). It logs one warning per task start, "Rate limiting could not determine a client IP", and nothing else. The same resolver feeds `session.ipAddress`, which was empty for every production session.

`buildAuthConfig` reads `TRUSTED_PROXY_CIDR` (the VPC range, `var.vpc_cidr`) into `trustedProxies`, `auth.ts` passes it through, and production refuses to boot without it (`startup-config.ts`). With it set the chain is walked from the right and the first entry outside the range is the viewer; a viewer can prepend whatever they like and it is never reached. `src/lib/__tests__/trusted-proxies.test.ts` pins that walk. The counter is per task, in memory (no `secondaryStorage`), so the limit multiplies by the task count; the service runs one. Confirm a deploy fixed it by the warning no longer appearing in `/ecs/eecs-capstone` on the next task start and by `session.ipAddress` holding distinct public addresses; one `10.x` value for everyone means the VPC origin ENI is outside the range. See the `trustHost` quirk above for the same chain from the origin-check side.
