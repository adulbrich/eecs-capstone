...
`pickup_by` / `due_at` on `inventory_request_items` and `current_pickup_by` / `current_due_at` on `inventory_items` are informational only. There is no cron. The "past pickup window" / "overdue" badges are computed at query time. Lazy idempotent notifications are inserted on read via `recordOverdueNotificationsAs`, which runs two scans: request lines with `status = 'approved'` (scoped to the viewer's own requests), and staff holds (`current_request_item_id IS NULL`, `current_holder_id IS NOT NULL`, `status IN ('reserved', 'checked_out')`), both folded into one `values` array for a single insert. The two scans are disjoint by construction: the hold scan requires `current_request_item_id IS NULL` and the request-line scan only fires for rows that have one, so no item can match both in the same pass. A partial unique index `notifications_overdue_unique_idx` on `(user_id, type, link)` for the two overdue types, keyed on `/inventory/${itemId}` rather than the request line, lets `onConflictDoNothing` collapse re-reads (the same scan producing the same row again on a later call) into the same key space; it is a safety net against repeated reads, not the mechanism that keeps the two scans from overlapping. The target + where are declared explicitly so future unique indexes on `notifications` cannot silently swallow unrelated conflicts.
