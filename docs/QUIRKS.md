...
### Mentorship is a three-state column and an address, two derived flags, and the mark is the proposer's

`projects.mentor_need` (`unspecified`, `seeking`, `none`; the tuple is `MENTOR_NEEDS` in `src/lib/vocabularies.ts`) and `projects.mentor_email` are written only by `updateProjectMentorshipAs`; `projects.student_proposed` is written only by `updateProjectProposerAs`, beside the link, because it says who proposed the project (#336). None of the three is on `ProjectInput`, so `updateProjectAs` cannot touch them. The mentor is resolved at read time by `mentorNameSql`, a case-insensitive correlated subquery with `LIMIT 1` rather than a `LEFT JOIN`, because a join on `lower(email)` would fan a project out into two rows if two accounts differed only by case. There is no `mentor_id`: mentorship grants no permission. The public sees two flags derived in SQL, never the state: `seekingMentor` is `mentor_need = 'seeking' AND mentor_email IS NULL`, because the public payload does not carry the address and a client cannot otherwise tell "no mentor" from "a mentor who has not signed up yet"; the second state shows nothing, on purpose, and the address guard is also what keeps a stale state from showing a badge beside a recorded mentor. `noMentorNeeded` is `mentor_need = 'none'` with no address guard, because the writer refuses `none` beside an address in either order and writes nothing, so the two cannot disagree ([ADR-0022](./adr/0022-mentorship-is-a-three-state-column-that-excludes-an-address.md)); the two messages come from `mentorNeedRefusal` in `src/lib/mentor-need.ts`, which the Mentor section also renders under the draft against the saved state, so the line staff read before Save is the line Save would throw (#373). `searchProjectsImpl` and `buildAdminProjectScope` use the same two expressions for `seekingMentorOnly` and `noMentorNeededOnly`, so a filter and its badge cannot disagree either. `student_proposed` is no input to either derivation since #304; a student project may need no mentor and a partner project may want one. Migration `0025` backfilled the old boolean from the mark and `0027` carried `seeking_mentor = true` over as `seeking`, so no badge changed on either deploy. Nothing about the mentor is public: `mentor_email`, the raw `mentor_need` and the resolved name leave the server through `getProjectMentorship`, the edit log, and `adminProjectSummarySelect` (the staff list and the CSV export, which carries the state beside the two derived columns) only; `projectSummarySelect` and `projectDetailView` carry neither, and the key-set pins hold it there. The analytics "published without mentor" count excludes the `none` state, since those projects are not missing anything. The Mentor section is a three-option radio group beside the address; it shows the mentor's linked or no-account state through the same `AccountLinkSummary` the Proposer section uses, and previews the badges from the draft with the same `ProjectBadges` the card renders. Both sections email a new address on save (the mentor gets no bell row, since they may have no account; the new proposer gets one), and both open `SendEmailDialog` with the skip only when the pending change would send: the address changed to a non-empty value, compared lowercased as the server compares, so a case-only retype or the state alone saves without a confirm (#379, #385). See #75, #304, #336, #373.
