...
Nothing records a failed automatic AI write except the app's own log, so the `ai_write_failures` metric filter in `infra/alarms.tf` counts log lines by their exact, case-sensitive wording. That is the coupling the pool metrics have (Drizzle section): reword a counted line and the filter matches nothing, so the alarm sits in OK through an outage and nothing fails anywhere, and lowercasing a line it leaves out can make one failure count twice. `src/server/__tests__/project-refresh.test.ts` runs the pattern against the lines the writers actually print, the ones it must not count included, so a rewording on either side is a red unit test. Change a message and the pattern in the same commit; the comment on the filter says which phrase matches which line. The `aws logs tail` recipe in `DEPLOYMENT.md` that lists them after a mail carries its own copy of the phrases, and no test checks it, so it changes in the same commit too.
