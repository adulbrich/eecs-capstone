...
### The production pool keeps five connections open, and the entry loads it

`poolConfig(url, { keepWarm: true })` sets `min: POOL_MIN` and `allowExitOnIdle`, and `src/db/index.ts` passes it and calls `warmPool` only when `NODE_ENV` is `production`. `min` exempts clients from the idle timeout but opens none, so `warmPool` opens them all at once: a connect-then-release loop gets the same idle client back and leaves one open. `src/server.ts` imports `#/db` for that side effect, because `/api/healthz` imports nothing from the database and the server functions reach `#/db` through their dynamic imports, so the pool would otherwise first open on the first page view, which can be inside a burst; with the import, a new task's first health check opens the floor. Measured on the built output against docker Postgres: five backends after `/api/healthz`, still five after 20 idle seconds. [ADR-0052](./adr/0052-the-pool-keeps-a-warm-floor.md) is why. #601.
