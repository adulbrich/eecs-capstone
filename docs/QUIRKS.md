...
`better-auth/api` exports an `APIError`, and the sign-in endpoint throws it, but better-call throws its OWN `APIError` (`better-call/dist/endpoint.mjs`) when a request body fails its zod schema. So in `hooks.after`, `ctx.context.returned instanceof APIError` is true for a wrong password and **false for a malformed body**, and code that reads "not an error, therefore a success" treats a request with a required field omitted as a successful sign-in. In #552 that was a complete bypass of the attempt counter: four wrong passwords, then one request with `password` missing, and the count was cleared. Use `isAPIError` from `better-auth/api`, which is what Better Auth's own dispatcher uses, and prefer detecting success positively (the endpoint returns an object carrying `user`) over inferring it from the absence of an error. Related: an after-hook runs even when the endpoint threw, which is what makes counting failures there possible at all, and is undocumented; `src/lib/__tests__/sign-in-attempts.integration.test.ts` pins both.
...
### `reset-password` does not set `emailVerified`, and `onPasswordReset` is where to

`better-auth/dist/api/routes/password.mjs` writes the new credential and then calls `onPasswordReset`, so by the time the hook runs the password is already the new one. It never touches `emailVerified`, which is what made account recovery a four-step maze in #554: a person who had just consumed a token mailed to their own address was still refused at sign-in as unverified. `markAddressProven` in `src/server/_internal/mark-address-proven.ts` runs in that hook and records the proof. Note also that `requestPasswordReset` is NOT gated on `emailVerified`, which is what makes the path usable as a recovery route at all, and that `revokeSessionsOnPasswordReset` defaults to false and is not enabled here.

### `captureStderr` sees the console email transport but not `console.*`

`ConsoleEmailSender` writes with `process.stderr.write`, which `src/test/shared/console-email.ts` patches, so `captureConsoleEmail` works. A `console.warn` in the same block does not reach that patch, because Vitest replaces the console methods with its own interception and reports them separately. A test that wants both, as `verification-sends.integration.test.ts` does, needs `captureStderr` for the mail and a `vi.spyOn(console, "warn")` for the line.
