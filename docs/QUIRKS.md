...
### Mentorship is three staff-written columns and one derived flag

`projects.student_proposed`, `projects.seeking_mentor` and `projects.mentor_email` are written only by `updateProjectMentorshipAs`; none is on `ProjectInput`, so `updateProjectAs` cannot touch them. The mentor is resolved at read time by `mentorNameSql`, a case-insensitive correlated subquery with `LIMIT 1` rather than a `LEFT JOIN`, because a join on `lower(email)` would fan a project out into two rows if two accounts differed only by case. There is no `mentor_id`: mentorship grants no permission. The public `seekingMentor` is `seeking_mentor AND mentor_email IS NULL`, computed in SQL, because the public payload does not carry the address and a client cannot otherwise tell "no mentor" from "a mentor who has not signed up yet"; the second state shows nothing, on purpose, and the address guard is also what keeps a stale flag from showing a badge beside a name. `student_proposed` is no input to the derivation since #304 (migration `0025` backfilled the flag from it, so no badge changed on deploy); a student project may need no mentor and a partner project may want one. `mentor_email` and the raw `seeking_mentor` leave the server through `getProjectMentorship` and the edit log only. The staff panel section shows the mentor's linked or no-account state through the same `AccountLinkSummary` the Proposer section uses. See #75, #304.
