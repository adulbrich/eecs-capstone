...
Better Auth validates the body inside the endpoint, after every before-hook has run, so a before-hook reads the raw JSON: a missing field, a value of the wrong type, an address with a leading space. And an after-hook runs when the handler threw, with the error in `ctx.context.returned`. Together that made three holes in the code guards in `src/lib/auth.ts` (app-security-review on #576): a send with no `type` spent the owner's per-recipient budget and was then handed a claim on the owner's live code, unrotated; a padded address spent the budget, because the cap trims and Better Auth does not, and mailed nothing; and a redeem with no `otp` answered differently depending on whether a code was outstanding. `refusedByBetterAuth` now makes every hook leave such a body alone so Better Auth answers with its own validation error, and the claim is issued only when `isAPIError(ctx.context.returned)` is false. A new hook on any auth path should start from the same question: what does this do with a body the handler is about to reject? The three cases in `email-otp.integration.test.ts` under "requests Better Auth refuses" pin it.
