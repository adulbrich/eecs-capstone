...
Deleting an account outright is not supported. Nine `onDelete: "restrict"` edges into `user.id` carry authorship and audit records, so the row has to survive; ban a real user, and see #84 (self-service) and #29 (staff-initiated) for the anonymize-in-place path. A one-off purge script for test accounts existed until 2026-09-01 and was removed unused.
