...
The database is a burstable class (`db.t4g.small`). Changing the instance class restarts it, and the restart throws away the accrued `CPUCreditBalance`: it read 288.0 five minutes before #529 resized `micro` to `small` and 0.0 five minutes after, and `CPUSurplusCreditBalance` started accruing immediately, which is the instance spending past a balance it does not have. Credits come back at a measured 18 per hour under this app's idle load, against a 288 cap, so a full balance is most of a day away. Nothing breaks at idle, because the app sits far under the 20% baseline the credits are measured against, but anything that drives real database CPU in the hours after a resize is spending surplus. Plan a resize away from anything heavy, and read the balance rather than the clock before a load test: [`load-tests/2026-09-20-term-start.md`](./load-tests/2026-09-20-term-start.md) is the run that hit this.
