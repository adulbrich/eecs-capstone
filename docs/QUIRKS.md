...
**`user.email` is not one of them, and its folds stay.** Better Auth lowercases that column itself, on all three paths that create an account here: `api/routes/sign-up.mjs:165` for email and password, `oauth2/link-account.mjs:101` for every OAuth provider, ONID and GitHub alike, and the admin plugin's `routes.mjs:191` for create-user, in 1.6.25. That makes the folds in `resolveHold`, `lookupUserByEmailAs`, `resolveProposerId` and `mentorNameSql` defensive rather than load-bearing, and they stay: an upgrade changing that behavior would otherwise stop linking accounts with nothing to show for it. Only a `lower()` applied to one of the four columns above may be dropped, and only after the migration has run against a deployed database, which is why this release drops none.
