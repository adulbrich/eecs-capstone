...
Do not read `canEdit` in that return value as authoritative. `getProjectAs:310-314` reimplements the rule inline and disagrees with `canEditProject` for exactly one case, staff on an archived project: the inline copy says no, the predicate says yes, and `project-visibility.test.ts:108` pins the predicate's answer. So the page hides the edit affordance while `updateProjectAs` would accept the write. Preserved verbatim through the projection change rather than silently resolved, because which one is correct is a product question. See issue #40.
