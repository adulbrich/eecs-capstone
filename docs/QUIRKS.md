...
- **`updateProjectProposerAs` is the only writer of the address after create.** Staff-only, one edit-log row per change, `proposerId` derived from the address and never from the client ([ADR-0007](./adr/0007-proposer-linking-by-email.md)); an empty string or `null` unlinks. `updateProjectAs` has no key for it, so a smuggled `proposerEmail` on an ordinary edit falls on the floor, which `projects.integration.test.ts` proves for staff as well.
