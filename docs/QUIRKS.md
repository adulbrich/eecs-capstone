...
### A listing's filters render twice, and the copies must not fight

`ListingLayout` mounts the `filters` element in the aside (display `none` below `xl`) and again inside the `Sheet` while it is open. So a filters component is mounted twice at the moment a reader below `xl` has the sheet open. Both copies are controlled by the same URL state and navigate on change, which is fine; what is not fine is a copy that owns something. Local draft state (a debounced search input) belongs in `search`, which renders once, not in `filters`. Fixed element ids inside `filters` are duplicated while the sheet is open; the aside copy is out of the accessibility tree then, and the a11y suite scans that state on both routes, so a new id is checked there rather than avoided by hand. If axe ever reports a duplicate, the fix is a render-prop `filters` that takes an id prefix, not a `useId` per switch, since the a11y tests name the switches and Playwright does not care about ids. ADR 0020 has the decision.
...
### Mentorship is two staff-written columns and one derived flag, and the mark is the proposer's

`projects.seeking_mentor` and `projects.mentor_email` are written only by `updateProjectMentorshipAs`; `projects.student_proposed` is written only by `updateProjectProposerAs`, beside the link, because it says who proposed the project (#336). None of the three is on `ProjectInput`, so `updateProjectAs` cannot touch them. The mentor is resolved at read time by `mentorNameSql`, a case-insensitive correlated subquery with `LIMIT 1` rather than a `LEFT JOIN`, because a join on `lower(email)` would fan a project out into two rows if two accounts differed only by case. There is no `mentor_id`: mentorship grants no permission. The public `seekingMentor` is `seeking_mentor AND mentor_email IS NULL`, computed in SQL, because the public payload does not carry the address and a client cannot otherwise tell "no mentor" from "a mentor who has not signed up yet"; the second state shows nothing, on purpose, and the address guard is also what keeps a stale flag from showing a badge beside a recorded mentor. `searchProjectsImpl` uses the same expression for `seekingMentorOnly`, so the filter and the badge cannot disagree. `student_proposed` is no input to the derivation since #304 (migration `0025` backfilled the flag from it, so no badge changed on deploy); a student project may need no mentor and a partner project may want one. Nothing about the mentor is public: `mentor_email`, the raw `seeking_mentor` and the resolved name leave the server through `getProjectMentorship`, the edit log, and `adminProjectSummarySelect` (the staff list and the CSV export) only; `projectSummarySelect` and `projectDetailView` carry neither, and the key-set pins hold it there. The Mentor section shows the mentor's linked or no-account state through the same `AccountLinkSummary` the Proposer section uses, and previews the badge from the draft with the same `MentorshipBadges` the card renders. See #75, #304, #336.
